...
%   heuristique2(U, H).  % ensuite utilisez plutot l'heuristique 2  
   ...
   coordonnees1(Piece,Etat,Lig,Col) :-  nth1(Lig,Etat,Ligne), nth1(Col,Ligne, Piece).
   malplace(P,U,F) :- not(vide==P),coordonnees1(P,U,Lig,Col), not (coordonnees1(P,F,Lig,Col)).
   ...
   % entre sa position courante et sa positon dans l'etat final
    ...
    coordonnees([L,C], Mat, Elt):-  nth1(L,Mat,L1), nth1(C,L1, Elt).
    dm([],U,0).
    dm([vide|R],U,RESU):-!, dm(R,U,RESU).
    dm([E1|R],U,RESU) :- coordonnees([L1,C1], U, E1), final_state(F), coordonnees([L2,C2], F, E1), T is (abs(L1-L2) + abs(C1-C2)),  dm(R,U,RES), RESU is RES+T, display(display(T)).
    
    heuristique2([],M,0).
    heuristique2([L1|R1],M,H) :- dm(L1, M,P), heuristique2(R1,M,H1), H is P+H1. 
